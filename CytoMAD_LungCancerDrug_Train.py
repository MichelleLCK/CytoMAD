...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 13 13:52:03 2019

@author: michellelo9774
"""
...
def define_mapping1(input_shape = (512,)):
	# weight initialization
	init = RandomNormal(stddev=0.02)
	# image input
	features_gen = Input(shape=input_shape)
    
	l1 = Dense(384, activation='relu', kernel_initializer=init)(features_gen)
	l2 = Dense(256, activation='relu', kernel_initializer=init)(l1)
	l3 = Dense(384, activation='relu', kernel_initializer=init)(l2)
	out = Dense(input_shape[0], activation='relu', kernel_initializer=init)(l3)
...
def generate_real_samples(dataset, n_samples, patch_shape):
	# unpack dataset
	trainA, trainB, drug_label, drug_onehot, _, cell_label, cell_onehot, _, batch_label, batch_onehot, _ = dataset
	# choose random instances
	ix = random.sample(range(trainA.shape[0]), n_samples)
	# retrieve selected images
	X1, X2, D1, D_onehot, C1, C_onehot, B1, B_onehot = trainA[ix,:,:,:], trainB[ix,:,:,:], drug_label[ix,:], drug_onehot[ix,:], cell_label[ix,:], cell_onehot[ix,:], batch_label[ix,:], batch_onehot[ix,:]
	# generate 'real' class labels (1)
	y = ones((n_samples, patch_shape, patch_shape, 1))
	return [X1, X2, D1, D_onehot, C1, C_onehot, B1, B_onehot], y
...
def generate_real_samples_all(dataset, patch_shape):
	...
	trainA, trainB, drug_label, drug_onehot, _, cell_label, cell_onehot, _, batch_label, batch_onehot, _ = dataset
	# retrieve selected images
	X1, X2, D1, D_onehot, C1, C_onehot, B1, B_onehot = trainA, trainB, drug_label, drug_onehot, cell_label, cell_onehot, batch_label, batch_onehot
	# generate 'real' class labels (1)
...
def summarize_performance(step, g_model, dataset, Path_Figures, Path_MP, n_samples=3):
	...
	[X_realA, X_realB, _, drug_onehot, _, cell_onehot, _, batch_onehot], _ = generate_real_samples(dataset, n_samples, 1)
...
def summarize_performance_all(step, d_model, g_model, cell_model_CNN, cell_model, batch_model_CNN, batch_model1, batch_model2, batch_model3, batch_model4, RandomFeaturesList1, RandomFeaturesList2, RandomFeaturesList3, RandomFeaturesList4, dataset, Path_Figures_CytoMAD, Path_MP_CytoMAD, n_samples=3):
	...
	[X_realA, X_realB, _, _, _, _, _, _], _ = generate_real_samples(dataset, n_samples, 1)
	# generate a batch of fake samples
	X_fakeB, features_gen, _ = generate_fake_samples(g_model, X_realA, 1)
	features_gen = np.squeeze(features_gen, axis=2)
	features_gen = np.squeeze(features_gen, axis=1)
	...
	[X_realA, _, _, drug_onehot, _, cell_onehot, _, batch_onehot], _ = generate_real_samples(dataset, dataset[0].shape[0], 1)
	X_fakeB, features_gen, _ = generate_fake_samples(g_model, X_realA, 1)
	features_gen = np.squeeze(features_gen, axis=2)
	features_gen = np.squeeze(features_gen, axis=1)
...
def load_multi_dataset(BasePath, Date, Cells, Samples, state, cellsCount):
	...
	for i in range(len(Date)):
		...
		DrugLabel = np.ones((cellsCount[i],1))*DrugType
	...
	shuffled_DrugLabels_index = DrugLabels[RandomList]
...
def train(d_model, g_model, cell_model, cell_model_CNN, batch_model_CNN, gan_model, dataset, dataset_valid, timestr, NoOfBatch, batch_Lrate, batch_input_shape, Path_Figures, Path_Figures_CytoMAD, Path_MP, Path_MP_CytoMAD, n_epochs1=100, n_epochs2=100, n_epochs3=300, epoch_interval = 5, n_batch=10):
	...
	trainA, trainB, train_drug, train_drug_onehot, drug_dict, train_cell, train_cell_onehot, cell_dict, train_batch, train_batch_onehot, batch_dict = dataset
	testA, testB, test_drug, test_drug_onehot, _, test_cell, test_cell_onehot, _, test_batch, test_batch_onehot, _ = dataset_valid
	...
	[X_realA_test, X_realB_test, _, drug_onehot_test, _, cell_onehot_test, _, batch_onehot_test], y_real_test = generate_real_samples(dataset_valid, n_batch, n_patch)
	g_loss_valid_reference = 10000
	for i in range(n_steps):
		# select a batch of real samples
		[X_realA, X_realB, _, drug_onehot, _, cell_onehot, _, batch_onehot], y_real = generate_real_samples(dataset, n_batch, n_patch)
		# generate a batch of fake samples
	...
	[train_X_realA, _, _, train_drug_onehot, _, train_cell_onehot, _, train_batch_onehot], y_real = generate_real_samples_all(dataset, n_patch)  
	[test_X_realA, _, _, test_drug_onehot, _, test_cell_onehot, _, test_batch_onehot], y_real = generate_real_samples_all(dataset_valid, n_patch)
	...
	features_gen = np.squeeze(features_gen, axis=2)
	features_gen = np.squeeze(features_gen, axis=1)
	print('features_gen:', features_gen.shape)
	...
	for i in range(n_steps):
		...
		if i%n_interval == 0:
			...
			[X_realA, _, _, drug_onehot, _, cell_onehot, _, batch_onehot], y_real = generate_real_samples_all(dataset, n_patch)  
...
###### Load Image Data ######
# Define the batch
Date = ['B1', 'B1', 'B1', 'B1', 'B1', 'B1', 'B2', 'B2', 'B2', 'B2', 'B2', 'B2', 'B3', 'B3', 'B3', 'B3', 'B3', 'B3', 'B4', 'B4', 'B4', 'B4', 'B4', 'B4', 'B5', 'B5', 'B5', 'B5', 'B5', 'B5', 'B6', 'B6', 'B6', 'B6', 'B6', 'B6']
NoOfBatch = len(set(Date))
# Define the Drug Type
Cells = ['DMSO', 'Docetaxel', 'Docetaxel', 'Docetaxel', 'Docetaxel', 'Docetaxel', 'DMSO', 'Docetaxel', 'Docetaxel', 'Docetaxel', 'Docetaxel', 'Docetaxel', 'DMSO', 'Afatinib', 'Afatinib', 'Afatinib', 'Afatinib', 'Afatinib', 'DMSO', 'Afatinib', 'Afatinib', 'Afatinib', 'Afatinib', 'Afatinib', 'DMSO', 'Gemcitabine', 'Gemcitabine', 'Gemcitabine', 'Gemcitabine', 'Gemcitabine', 'DMSO', 'Gemcitabine', 'Gemcitabine', 'Gemcitabine', 'Gemcitabine', 'Gemcitabine']
NoOfCellType = len(set(Cells))
# Define the Drug Concentration
Samples = ['Control','D1','D2','D3','D4','D5', 'Control','D1','D2','D3','D4','D5', 'Control','A1','A2','A3','A4','A5', 'Control','A1','A2','A3','A4','A5', 'Control','G1','G2','G3','G4','G5', 'Control','G1','G2','G3','G4','G5']
NoOfDrugType = len(set(Samples))
# No of cells per class during training
cellsCount = [1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000]
# Load training dataset
state = 'Train'
dataset = load_multi_dataset(BasePath, Date, Cells, Samples, state, cellsCount)
print('Loaded', dataset[0].shape, dataset[1].shape)

# No of cells per class in validation
CellsPerCellLines_test = [500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500, 500]
# Load validation dataset
state = 'Valid'
dataset_valid = load_multi_dataset(BasePath, Date, Cells, Samples, state, CellsPerCellLines_test)
...
cell_model = define_classifier(NoOfCellType, Lrate = 0.0001)

# Define the Convolutional-Neural-Network-based Batch Classifier and Cell Type Classifier
batch_model_CNN = define_CNN(input_shape, NoOfBatch, Lrate = 0.0001)
cell_model_CNN = define_CNN(input_shape, NoOfCellType, Lrate = 0.0001)
